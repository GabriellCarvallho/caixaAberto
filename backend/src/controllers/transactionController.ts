...
async function create(req: Request, res: Response, forcedType: TransactionType) {
  try {
    ...
    if (typeof description !== 'string' || description.trim().length === 0 || description.length > 5000) {
      return res.status(400).json({ error: 'description deve ser um texto entre 1 e 5000 caracteres.' })
    }
    ...
    if (!organization.managementStart || date < organization.managementStart || date > organization.managementEnd) {
      return res.status(400).json({ error: 'A data do lançamento deve estar dentro do período de gestão da organização.' })
    }
...
export async function monthlySummary(req: Request, res: Response) {
  try {
    ...
    const [currentGroups, previousGroups] = await Promise.all([
      ...
      transactionRepository.getMonthlySummary(organizationId, range.previousStart, range.previousEnd),
    ])
...
export async function publicTransparency(req: Request, res: Response) {
  try {
    ...
    if (!organization) {
      return res.status(404).json({ error: 'Página de transparência não encontrada ou desativada.' })
    }
    ...
    const categoryIds = [...new Set(groups.map((group) => group.categoryId.toString()))].map(BigInt)
